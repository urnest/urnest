...
PATH="$ODIN_SQL_PATH" "$ODIN_SQL_CLIENT" "$ODIN_SQL_DB" "$ODIN_SQL_USER" "$ODIN_SQL_PASSWORD" < "$ODIN_sql" >sql.stdout 2>WARNINGS || {
  mv WARNINGS ERRORS;
}
bzip2 -c "$ODIN_sql" > load
test -z "$ODIN_pre" || cat "$ODIN_pre" |
while read n
do
  cat "$n" >> load
done
