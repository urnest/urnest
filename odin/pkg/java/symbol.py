...
def main(argv):
    ...
    s=file(ODIN_classnames).read().splitlines()[0].split()[1]
    if ODIN_java==s:
        assert s.endswith('.java')
        s=os.path.basename(s)[0:-5]
    print package+'.'+s
    if len(ODIN_syms):
        print string.join(file(ODIN_syms).read().split(), '\n')
    if len(ODIN_syms_file):
        print string.join(file(ODIN_syms_file).read().split(), '\n')
        
if __name__=='__main__':
    try:
        if os.environ.has_key('ODINVERBOSE'):
            print string.join(sys.argv, ' ')
        sys.stdout=file('symbol', 'w')
        main(sys.argv)
    except:
