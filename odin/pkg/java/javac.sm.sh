...
if [ "$ODINVERBOSE" != "" ] ; 
then
   echo "${ODINRBSHOST} javac -d $ODIN_JAVA_OUTPUT_DIRECTORY $debug $classpath" `cat $ODIN_sources` 2>&1
fi
...
do
  ...
  then
    ...
    echo "$classname.java" >> sources
  else
    echo "$source" >> sources
  fi
done

PATH="$ODIN_JAVA_PATH" javac -implicit:none -d output $debug $classpath `cat sources` >WARNINGS 2>&1 || {
  mv WARNINGS ERRORS;
  rm -r output;
  exit 0;
}
