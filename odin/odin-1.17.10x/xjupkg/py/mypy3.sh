...
(
  eval env - LD_LIBRARY_PATH="$ODIN_MYPY_LD_LIBRARY_PATH" PATH="$ODIN_MYPY_PATH" PYTHONPATH="$PYPATH:$ODIN_PYTHON3PATH" MYPYPATH="$PYPATH:$ODIN_MYPYPATH" `cat "$ODIN_env"` "$ODIN_MYPY" $ODIN_FILE &&
  touch mypy3
