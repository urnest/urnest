...
(() => {
    ...
    try {
        ...
        try{
            xju.assert.assertXcontainsY(xju.str(e), 'failed to validate property a_type_alias because failed to verify null is a Array<int|null> because null is not an array it is a object');
        }
        catch(ee){
            errors.push(`${ee}`);
        }
    ...
    xju.assert.assertEqual(xju.json_codec.asInstanceOfTypeScriptNamespace({ defs:{ "a": "true;" } }), { defs:{ "a": "true;" }, namespaces_this_namespace_depends_on: [] });
    xju.assert.assertEqual(xju.json_codec.isInstanceOfTypeScriptNamespace({ defs:{ "a": "true;" }, namespaces_this_namespace_depends_on: [] }), true);
    ...
    xju.assert.assertEqual(isInstanceOfTA([7]),true);
    xju.assert.assertEqual(asInstanceOfTA([7]),[7]);
    xju.assert.assertEqual(xju.json_codec.isInstanceOfTypeScriptNamespaceFQN(["xju"]),true);
    xju.assert.assertEqual(xju.json_codec.isInstanceOfTypeScriptFQN(["xju"]),true);
    xju.assert.assertEqual(xju.json_codec.asInstanceOfTypeScriptNamespaceFQN(["xju"]),["xju"]);
    xju.assert.assertEqual(xju.json_codec.asInstanceOfTypeScriptFQN(["xju"]),["xju"]);
})();
