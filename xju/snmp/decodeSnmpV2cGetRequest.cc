...
// implied warranty.
//
#include "decodeSnmpV2cGetRequest.hh"
#include <string>
...
std::pair<SnmpV2cGetRequest,std::vector<Oid> > decodeSnmpV2cGetRequest(
    ...
    xju::Exception)*/
{
  ...
  try {
    ...
    try {
      ...
      try {
        auto const community(decodeStringValue(snmpVersion.second)); try {
          ...
          if (s2.first.first!=0xA0) {
            RequestTypeMismatch e(s2.first.first,0xA0,XJU_TRACED);
            std::ostringstream s;
    ...
    std::ostringstream s;
    s << "decode snmp v2c get request from " 
