...
// implied warranty.
//
#include "decodeSnmpV1SetRequest.hh"
#include <string>
    ...
    xju::Exception)
{
  ...
  try {
      ...
      try {
        auto const community(decodeStringValue(snmpVersion.second)); try {
          ...
          if (s2.first.first!=0xA3) {
            throw RequestTypeMismatch(s2.first.first,0xA3,XJU_TRACED);
          }
          try {
              ...
              auto const errorStatus(decodeIntValue(id.second)); try {
                auto const ei(decodeIntValue(errorStatus.second)); try {
                  auto const s3(decodeSequenceTypeAndLength(ei.second)); try {
                    ...
                    std::map<Oid, std::shared_ptr<Value const> > vars;
    ...
    std::ostringstream s;
    s << "decode snmp v1 set request from " << data.size() << " bytes of data";
    if (ok.size()) {
