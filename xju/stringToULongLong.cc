...
// implied warranty.
//
#include <xju/stringToULongLong.hh>
                                     ...
                                     unsigned int base) throw(xju::Exception)
{
  try
  {
    ...
    char* p(0);
    const unsigned long long x(strtoull(value.c_str(), &p, base));
    if (*p != 0)
    {
      std::ostringstream cause;
    ...
    const unsigned long long result(x);
    if ((long)result != x)
    {
      std::ostringstream cause;
      cause << x 
            << " is too large (maximum allowed unsigned long long is "
            << ULLONG_MAX
            << ")";
      throw xju::Exception(cause, XJU_TRACED);
    }
  ...
  catch(xju::Exception& e)
  {
    ...
    s << "convert " << xju::format::quote(value) << " to a base-"
      << base << " unsigned long long integer";
