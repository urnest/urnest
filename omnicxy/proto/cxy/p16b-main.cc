...
class AAA_impl : public virtual p16::AAA
{
public:
  // AAA::
  int32_t f(
...
public:
  ...
  struct Call_f2 : Call
  {
    ...
    friend bool operator==(Call_f2 const& x, Call_f2 const& y) throw()
    {
      return x.a_==y.a_;
    }
  };
...
int main(int argc, char* argv[])
{
  try {
    ...
    if (argv[2]==std::string("client")) {
      ...
      CCC_impl ccc;
      cxy::sref<p16::CCC> const xccc(orb, OBJECT_NAME+"ccc", ccc);
    ...
    else
    {
      ...
      CCC_impl x;
      cxy::sref<p16::CCC> const xa(orb, OBJECT_NAME+"ccc", x);
